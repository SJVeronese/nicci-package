...
def importparameters(parameterfile):
    ...
    parameters['robust']=config.getfloat('CUBEPAR','robust') if config.has_option('CUBEPAR','robust') else None #robust parameter of the data cube. If it is not given, set it to None
...
#############################################################################################
def chanmap(datacube='',rms=None,from_chan=0,to_chan=None,chansep=1,chanmask=False,chanmapoutdir='',chanmapoutname='',save=False,**kwargs):
    ...
    Kwargs:
        ...
        contours (list/array): contours level. They will replace the default levels                                    
        chansig (float): lowest contour level in terms of chansig*rms
    ...
    if 'vsys' in kwargs: #if the beam major axis is in the input kwargs
        ...
        vsys=0 #set it to 0
        warnings.warn('No systemic velocity is given:set it to 0 km/s!')
    ...
    with fits.open(datacube) as cube: #open the data cube
        ...
        cube.close() #close the fits file
    ...
    if spectralres is None or rms is None or bmaj is None or bmin is None: #if one or more cube parameters are not given
        if 'nsigma' in kwargs: #check if nsgima keyword is in the input kwargs. If so, pass it through the **kargs, else, pass it explicitly
            ...
            stats=cubestat(datacube=datacube,spectralres=spectralres,rms=rms,nsigma=chansig,bmaj=bmaj,bmin=bmin,bpa=bpa,verbose=False) #calculate the statistics of the cube
        if spectralres is None: #if spectralres is not given
            spectralres=stats['spectral resolution'] #take the value from the cubestat results
        ...
        if bmaj is None: #if bmaj is not given
            bmaj=stats['beam major axis'] #take the value from the cubestat results
        if bmin is None: #if bmin is not given
            bmin=stats['beam minor axis'] #take the value from the cubestat results
        if bpa is None: #if bpa is not given
            bpa=stats['beam position angle'] #take the value from the cubestat results
    ...
    if 'CUNIT3' in header: #if the spectral unit is in the header
        ...
        specunits='km/s' #set the first channel to 0 km/s
        if verbose:
            warnings.warn('No spectral unit was found: spectral unit set to km/s!')
    ...
    if chanmask: #if a mask must be used
        with fits.open(maskcube) as maskcube: #open the mask cube
            ...
            maskcube.close() #close the fits file
    ...
    leftmargin=xlim[0]+10 #left margin for annotations position
    bottommargin=ylim[0]+10 #bottom margin for annotations position
    topmargin=ylim[1]-24 #top margin for annotations position
    #----------   NORMALIZATION  ---------#
    vmin=0.15*np.nanmin(data) #set the lower limit for the normalization
    vmax=0.15*np.nanmax(data) #set the upper limit for the normalization
    norm=cl.PowerNorm(gamma=0.5,vmin=vmin,vmax=vmax) #define the data normalization
    #------------   CONTOURS  ------------#
    ...
    for i in range(nrows):
        for j in range(ncols):
            ...
            chanmap=data[chans[k],:,:] #select the channel
            im=ax.imshow(chanmap,cmap='Greys',norm=norm,aspect='auto') #plot the channel map in units of detection limit
            if chanmask: #if a mask must be used
                ...
                ax.contour(chanmap*mask[chans[k],:,:]/rms,levels=ctr,cmap='Greys_r',linewidths=ctr_width,linestyles='solid') #add the contours within the mask
            else:
        ...
        if operation == 'clip': #if the datacube must be clipped
            ...
            if usemask: #if a mask is used
                with fits.open(mask2d) as Mask:
                    ...
                    Mask.close() #close the fits file
...
#############################################################################################
def cubestat(datacube='',spectralres=None,rms=None,nsigma=3,bmaj=None,bmin=None,bpa=None,**kwargs):
    ...
    Args:
        ...
        spectralres (float): cube spectral resolution in km/s
        rms (float): rms of the data cube in Jy/beam as a float. If not given (None), the function tries to calculate it
        nsigma (float): sigma of the detectin limit in terms of nsigma*rms
        bmaj (float): beam major axis in arcsec
        bmin (float): beam minor axis in arcsec
        bpa (float): beam position angle in degree
    ...
    with fits.open(datacube) as cube: #open the data cube
        ...
        elif bmaj is None and 'BMAJ' not in header: #if the beam major axis is not given and the keyword is not in the cube header
            if len(cube)>1: #if the cube as more extensions
                bmaj=np.nanmedian(cube[1].data['BMAJ']) #the beam major axis is the median of the major axis array
            else:
                bmaj=1 #beam major axis is set to 1 arcsec
                if verbose:
                    warnings.warn('No beam major axis information was found: major axis set to 1 arcsec!')
        ...
        elif bmin is None and 'BMIN' not in header: #if the beam minor axis is not given and the keyword is not in the cube header
            if len(cube)>1: #if the cube as more extensions
                bmin=np.nanmedian(cube[1].data['BMIN']) #the beam minor axis is the median of the minor axis array
            else:
                bmin=1 #beam minor axis is set to 1 arcsec
                if verbose:
                    warnings.warn('No beam minor axis information was found: minor axis set to 1 arcsec!')
        if bpa is None and 'BPA' in header: #if the beam position angle is not given and the keyword is in the cube header
            bpa=header['BPA'] #get the value from the cube header
        elif bpa is None and 'BPA' not in header: #if the beam position angle is not given and the keyword is not in the cube header
            if len(cube)>1: #if the cube as more extensions
                bpa=np.nanmedian(cube[1].data['BPA']) #the beam position angle is the median of the position angle array
            else:
                bpa=0 #beam position angle is set to 0 deg
                if verbose:
                    warnings.warn('No beam position angle information was found: position angle set to 0 deg!')
        cube.close() #close the fits file
    beamarea=1.13*(bmin*bmaj) #calculate the beam area
    #------------   SPECTRAL UNITS     ------------#
    if 'CUNIT3' in header: #if the spectral units keyword exists
        specunits=header['CUNIT3'] #store the spectral units from the cube header
    else:
        specunits='km/s' #set the first channel to 0 km/s
        if verbose:
            warnings.warn('No spectral unit was found: spectral unit set to km/s!')
    units=header['BUNIT'] #store the flux units from the cube header
    #------------   SPECTRAL RESOLUTION     ------------#
    if spectralres is None: #if the spectral resolution is not given
        if 'CDELT3' in header: #if the header has the spectral resolution
            spectralres=header['CDELT3'] #store the spectral resolution from the cube header
            if spectralres is None: #it might happen that the spectral resolution is negative. In that case revert it
                spectralres=-spectralres
            if specunits == 'm/s': #if the spectral units are m/s
                spectralres=spectralres/1000 #convert the resolution to km/s
        else:
            spectralres=1 #set it to 1 km/s
            if verbose:
                warnings.warn('No spectral resolution information was found: resolution set to 1 km/s!')
    #------------   RMS     ------------#    
    ...
    sensitivity=nsigma*rms #the detection limit is the rms
    sens_error=nsigma*rms_error #the detection limit error is the rms error
    
    #WE STORE THE RESULT IN A DICTIONARY#
    result={} #initialize the dictionary result
    result['beam major axis']=bmaj
    result['beam minor axis']=bmin
    result['beam position angle']=bmin
    result['beam area']=beamarea
    result['spectral resolution']=spectralres
    ...
    if verbose: #if print-to-terminal option is true
        ...
        print('The median rms per channel is: {:.2e} {}'.format(rms,units))
        print('The {}\u03C3 1-channel detection limit is: {:.2e} {}'.format(int(nsigma),sensitivity,coldenunits))
    return result
...
def fitsarith(path='',fits1='',fits2='',fitsoperation=None,fitsarithoutdir='',fitsarithoutname='',**kwargs):
    ...
    with fits.open(fits1) as Fits1: #open the first fits file
        ...
        with fits.open(fits2) as Fits2: #open the second fits file
            ...
            Fits2.close() #close the fits file
        ...
        Fits1.close() #close the fits file
...
def gaussfit(cubetofit='',gaussmask='',gaussindir='',spectralres=None,linefwhm=15,amp_thresh=0,p_reject=1,
    ...
    if mask2d == '' or mask2d is None: #if a data cube is not given
        if 'mask2d' in kwargs: #if the 2D mask is in kwargs
            if kwargs['mask2d'] == '' or kwargs['mask2d'] is None:
                raise ValueError('ERROR: 2D mask is not set: aborting!')
            else:
    ...
    with fits.open(datacube) as cube: #open the data cube
        ...
        with fits.open(mask2d) as mask2d: #open the 2D mask
            ...
            if mask.shape[0] != data.shape[1] or  mask.shape[1] != data.shape[2]: #if the mask cube has different size than the data cube
                raise ValueError('ERROR: mask and data cube has different spatial shapes: {} the mask and ({},{}) the data cube. Aborting!'.format(mask.shape,data.shape[1],data.shape[2]))
            mask2d.close() #close the fits file
        ...
        if 'CUNIT3' in header: #if the spectral unit is in the header
            ...
            if verbose:
                warnings.warn('No spectral unit was found: spectral unit set to km/s!')
        if spectralres is None: #if the spectral resolution is not given
            if 'CDELT3' in header: #if the header has the spectral resolution value
                spectralres=header['CDELT3'] #store the spectral resolution from the cube header
                if specunits == 'm/s': #if the spectral units are m/s
                    spectralres=spectralres/1000 #convert the resolution to km/s
        if 'CRVAL3' in header: #if the header has the starting spectral value
...
def getpv(pvcube='',pvwidth=None,pvpoints=None,pvangle=None,pvchmin=0,pvchmax=None,pvoutdir='',
    ...
    Args:
        ...
        pvangle (float): position angle of the slice in degree. If not given, will be the object position angle
    ...
    elif len(pvpoints)==4: #if four points are given
        from_center=False #extract the slice between them
        pvpoints=ICRS([pvpoints[0],pvpoints[1]]*u.degree,[pvpoints[2],pvpoints[3]]*u.degree)
    else:
        raise ValueError('ERROR: wrong number of coordinates given {}, required 2 or 4. Aborting!'.format(len(pvpoints)))
    ...
    if plot: #if the slice must be plotted
        ...
        leftmargin=xlim[0]+10 #left margin for annotations position
        bottommargin=ylim[0]+10 #bottom margin for annotations position
        topmargin=ylim[1]-15 #top margin for annotations position
        #WE DEFINE THE NORMALIZATION#
        vmin=0.1*np.nanmin(data) #set the lower limit for the normalization
        vmax=0.3*np.nanmax(data) #set the upper limit for the normalization
        norm=cl.PowerNorm(gamma=0.5,vmin=vmin,vmax=vmax)
        ...
        #WE ADD THE CONTOURS#
        ax.contour(data/rms,levels=ctr,cmap='Greys_r',linewidths=ctr_width,linestyles='solid') #add the positive contours
        ...
        if pixelres is not None and asectokpc is not None: #if the pixel resolution and the arcsec-to-kpc conversion is given
            kpcline=10/(asectokpc*pixelres) #length of the 10 kpc line in pixel
            ax.hlines(topmargin-2,leftmargin,leftmargin+kpcline,color='black',linewidth=2) #add the 10 kpc line
            ax.annotate('10 kpc',(leftmargin+(kpcline/2),topmargin),ha='center')
        #WE ADD THE SUBTITLE
...
def plotmom(which='all',mom0map='',mom1map='',mom2map='',plotmomoutdir='',plotmomoutname='',save=False,**kwargs):
    ...
    Kwargs:
        ...
        use_cube (bool): use a data cube to get information like the rms, the spectral resolution and the beam if True. Note that in that case you must supply a data cube
        datacube (string): name or path+name of the fits data cube if use_cube is True
        spectralres (float): cube spectral resolution in km/s
        bmaj (float): beam major axis in arcsec
        bmin (float): beam minor axis in arcsec
        bpa (float): beam position angle in degree
        nsigma (float): lowest contour level in terms of nsigma*rms
        vsys (float): object systemic velocity in km/s
        pixelres (float): pixel resolution of the data in arcsec
        asectokpc (float): arcsec to kpc conversion to plot the spatial scale
        objname (string): name of the object
        subtitle (string): subtitle of the pv plot
        wcs (astropy.WCS): wcs as astropy.WCS object to use in the plot. It will replace the default one
        position (int): position of the subplot in the figure as triplet of integers (111 = nrow 1, ncol 1, index 1)
        rms (float): rms of the data cube in Jy/beam as a float. If not given (None), the function tries to calculate it
        mom0_ctr (list/array): contours level. They will replace the default levels
        lim (list/array): list or array of plot x and y limits as [xmin,xmax,ymin,ymax]. They will replace the default limits
        ctr_width (float): line width of the contours
        verbose (bool): option to print messages and plot to terminal if True   
    ...
    if outname == '' or outname is None: #if the outname is empty
        ...
        if which in ['all','mom1']: #if all moment maps must be plotted or only the moment 1
            outname=mom1map.replace('.fits','.pdf')  #the outname is the input .pdf
        if which in ['all','mom2']: #if all moment maps must be plotted or only the moment 2
    ...
    if use_cube: #if use cube
        stats=cubestat(datacube=datacube,spectralres=spectralres,rms=rms,nsigma=nsigma,bmaj=bmaj,bmin=bmin,bpa=bpa,verbose=False) #calculate the statistics of the cube
    ...
    if which == 'all' or which == 'mom0': #if all moment maps or moment 0 map must be plotted
        ...
        if pixelres is None: #if the pixelres is not given
            if 'CUNIT1' in mom0header: #if the spatial unit is in the header
                spaceunits=mom0header['CUNIT1'] #store the spatial unit
                if 'CDELT1' in mom0header: #if the spatial resolution is in the header
                    spaceres=abs(mom0header['CDELT1']) #store the spatial resolution from the header
                    if spaceunits == 'deg': #if the spatial unit is degree
                        pixelres=spaceres*3600 #convert into arcsec
                    elif spaceunits == 'arcmin': #if the spatial unit is arcmin
                        pixelres=spaceres*60 #convert into arcsec
    ...
    if which == 'all' or which == 'mom1': #if all moment maps or moment 1 map must be plotted
        ...
        #WE TRY TO GET THE PIXEL RESOLUTION         
        if pixelres is None: #if the pixelres is not given
            if 'CUNIT1' in mom1header: #if the spatial unit is in the header
                spaceunits=mom1header['CUNIT1'] #store the spatial unit
                if 'CDELT1' in mom1header: #if the spatial resolution is in the header
                    spaceres=abs(mom1header['CDELT1']) #store the spatial resolution from the header
                    if spaceunits == 'deg': #if the spatial unit is degree
                        pixelres=spaceres*3600 #convert into arcsec
                    elif spaceunits == 'arcmin': #if the spatial unit is arcmin
                        pixelres=spaceres*60 #convert into arcsec
    ...
    if which == 'all' or which == 'mom2': #if all moment maps or moment 2 map must be plotted
        ...
        #WE TRY TO GET THE PIXEL RESOLUTION# 
        if pixelres is None: #if the pixelres is not given
            if 'CUNIT1' in mom2header: #if the spatial unit is in the header
                spaceunits=mom2header['CUNIT1'] #store the spatial unit
                if 'CDELT1' in mom2header: #if the spatial resolution is in the header
                    spaceres=abs(mom2header['CDELT1']) #store the spatial resolution from the header
                    if spaceunits == 'deg': #if the spatial unit is degree
                        pixelres=spaceres*3600 #convert into arcsec
                    elif spaceunits == 'arcmin': #if the spatial unit is arcmin
                        pixelres=spaceres*60 #convert into arcsec
    ...
    if which == 'all' or which == 'mom0': #if all moment maps must be plotted or only the moment 0 map
        ...
        #WE DEFINE THE LIMITS#
        if lim is None: #if no axes limits are given
            xlim=[np.min(np.where(~np.isnan(mom0))[1])-5,np.max(np.where(~np.isnan(mom0))[1])+5] #x limits. 5 blank pixel to the side
            ylim=[np.min(np.where(~np.isnan(mom0))[0])-30,np.max(np.where(~np.isnan(mom0))[0])+5] #y limits. 5 blank pixel to the top and 30 to the bottom
        else: #get it from the input
            xlim=[lim[0],lim[1]]
            ylim=[lim[2],lim[3]]
        leftmargin=xlim[0]+10 #left margin for annotations position
        ...
        if momunit == 'cm$^{-2}$': #if the moment 0 is column density
            norm=cl.PowerNorm(gamma=0.5,vmin=-sens,vmax=0.75*np.nanmax(mom0)) #define the normalization from negative detection limit to 20% of the maximum
        else:
            norm=cl.PowerNorm(gamma=0.5,vmin=0,vmax=0.75*np.nanmax(mom0)) #define the normalization from negative detection limit to 20% of the maximum
        #WE DEFINE THE CONTOURS#
        ...
        #WE DO THE PLOT#
        im=ax.imshow(mom0,cmap='Greys',norm=norm,aspect='auto') #plot the moment 0 map
        ax.tick_params(direction='in') #change the ticks of the axes from external to internal
        ax.set_xlim(xlim) #set the xlim
        ax.set_ylim(ylim) #set the ylim
        ax.set_xlabel('RA') #set the x-axis label
        ...
        if which == 'all': #if all moment maps must be plotted
            if momunit == 'cm$^{-2}$': #if the moment 0 is column density
                cb=fig.colorbar(im,ax=ax,location='top',pad=0.0,label='HI column density [10$^{18}$ cm$^{-2}$]',shrink=1) #add the colorbar on top of the plot
            else:
                cb=fig.colorbar(im,ax=ax,location='top',pad=0.0,label='Flux [{}]'.format(momunit),shrink=1) #add the colorbar on top of the plot
        else:
            if momunit == 'cm$^{-2}$': #if the moment 0 is column density
                cb=plt.colorbar(im,ax=ax,location='top',pad=0.0,label='HI column density [10$^{18}$ cm$^{-2}$]',shrink=1) #add the colorbar on top of the plot
            else:
    ...
    if which == 'all' or which == 'mom1': #if all moment maps must be plotted or only the moment 0 map
        ...
        #WE DEFINE THE LIMITS#
        if lim is None: #if no axes limits are given
            xlim=[np.min(np.where(~np.isnan(mom1))[1])-5,np.max(np.where(~np.isnan(mom1))[1])+5] #x limits. 5 blank pixel to the side
            ylim=[np.min(np.where(~np.isnan(mom1))[0])-30,np.max(np.where(~np.isnan(mom1))[0])+5] #y limits. 5 blank pixel to the top and 30 to the bottom
        else: #get it from the input
            xlim=[lim[0],lim[1]]
            ylim=[lim[2],lim[3]]
        leftmargin=xlim[0]+10 #left margin for annotations position
        ...
        #WE DEFINE THE CONTOURS#
        ctr_res=35 #resolution of the velocity contours
        ctr_pos=np.arange(0,(np.nanmax(mom1)).astype(int),ctr_res) #positive radial velocity contours
        ctr_neg=np.flip(-np.arange(ctr_res,(np.abs(np.nanmin(mom1))).astype(int),ctr_res)) #negative radial velocity contours
        #WE DO THE PLOT#
        im=ax.imshow(mom1,cmap='bwr',norm=norm,aspect='auto') #plot the moment 1 map with a colormap centered on 0
        ax.tick_params(direction='in') #change the ticks of the axes from external to internal
        ax.set_xlim(xlim) #set the xlim
        ax.set_ylim(ylim) #set the ylim
        ax.set_xlabel('RA') #set the x-axis label
        ...
        #WE ADD THE COLORBAR#
        if which == 'all': #if it is an atlas
            cb=fig.colorbar(im,ax=ax,location='top',pad=0.0,label='Radial velocity [km/s]',shrink=1) #add the colorbar on top of the plot
        else:
            cb=plt.colorbar(im,ax=ax,location='top',pad=0.0,label='Radial velocity [km/s]',shrink=1) #add the colorbar on top of the plot
        ...
        #WE ADD ANCILLARY INFORMATION#
        ax.text(leftmargin,bottommargin,'Systemic velocity: v$_{{sys}}$ = {:.2f} km/s'.format(vsys)) #add the information of the systemic velocity
        if pixelres is not None and bmaj is not None and bmin is not None and bpa is not None: #if the pixel resolution and the beam is given
            pxbeam=np.array([bmaj,bmin])/pixelres #beam in pixel
            box=patch.Rectangle((xlim[1]-4*pxbeam[0],ylim[0]),4*pxbeam[0],4*pxbeam[1],fill=None) #create the box for the beam
            beam=patch.Ellipse((xlim[1]-2*pxbeam[0],ylim[0]+2*pxbeam[1]),pxbeam[0],pxbeam[1],bpa,hatch='/////',fill=None) #create the beam patch
    ...
    if which == 'all' or which == 'mom2': #if all moment maps must be plotted or only the moment 0 map
        ...
        #WE DEFINE THE LIMITS#
        if lim is None: #if no axes limits are given
            xlim=[np.min(np.where(~np.isnan(mom2))[1])-5,np.max(np.where(~np.isnan(mom2))[1])+5] #x limits. 5 blank pixel to the side
            ylim=[np.min(np.where(~np.isnan(mom2))[0])-30,np.max(np.where(~np.isnan(mom2))[0])+5] #y limits. 5 blank pixel to the top and 30 to the bottom
        else: #get it from the input
            xlim=[lim[0],lim[1]]
            ylim=[lim[2],lim[3]]
        leftmargin=xlim[0]+10 #left margin for annotations position
        ...
        #WE ADD THE COLORBAR#
        if which == 'all': #if all moment maps must be plotted
            cb=fig.colorbar(im,ax=ax,location='top',pad=0.0,label='Velocity dispersion [km/s]',shrink=1) #add the colorbar on top of the plot
        else:
            cb=plt.colorbar(im,ax=ax,location='top',pad=0.0,label='Velocity dispersion [km/s]',shrink=1) #add the colorbar on top of the plot
        cb.ax.tick_params(direction='in',length=5) #change the ticks of the colorbar from external to internal and made them longer
        cb.set_ticks(ctr) #set the ticks of the colobar to the levels of the contours
        #WE ADD ANCILLARY INFORMATION#
        ax.text(leftmargin,bottommargin,'Median dispersion: FWHM = {:.2f} km/s'.format(disp)) #add the information of the velocity dispersion
        if pixelres is not None and bmaj is not None and bmin is not None and bpa is not None: #if the pixel resolution and the beam is given
            pxbeam=np.array([bmaj,bmin])/pixelres #beam in pixel
            box=patch.Rectangle((xlim[1]-4*pxbeam[0],ylim[0]),4*pxbeam[0],4*pxbeam[1],fill=None) #create the box for the beam
            beam=patch.Ellipse((xlim[1]-2*pxbeam[0],ylim[0]+2*pxbeam[1]),pxbeam[0],pxbeam[1],bpa,hatch='/////',fill=None) #create the beam patch
    ...
    if which == 'all': #if all moment maps must be plotted
        fig.subplots_adjust(left=0.07, bottom=0.1, right=0.97, top=0.90, wspace=0.0, hspace=0.3) #fix the position of the subplots in the figure   
        if save: #if the save switch is true
            fig.savefig(outname,dpi=300,bbox_inches='tight') #save the figure
...
def removemod(datacube='',modelcube='',maskcube='',method='subtraction',blankthreshold=0,removemodoutdir='',
    ...
    with fits.open(datacube) as cube: #open the data cube
        ...
        with fits.open(modelcube) as modelcube: #open the model cube
            ...
            modelcube.close() #close the fits file
        if maskcube == '' or maskcube is None: #if a mask cube is not given
...
#############################################################################################
def getHImass(data,beamarea=None,pixelres=None,units=None,pbcorr=False,distance=None,box=None,**kwargs):
    ...
    if type(data) is str: #if the data as a path-to-file
        ...
        if beamarea is None or pixelres is None: #if no additional information are given, abort
            ValueError('ERROR: Please provide the spatial resolution, and the beam area. Aborting!')
        darray=data #store the data
    else:
        ValueError('ERROR: Please provide the data as path-to-fits file or as numpy.ndarray. Aborting!')
    ...
    if units is None: #if no units for the data are provided
        if type(data) is str: #if the data as a path-to-file
            ...
            raise ValueError('ERROR: No data units are found: aborting!')
    if 'km/s'.casefold() in units: #if the units are km/s
        pass #do nothing
    elif 'm/s'.casefold() in units: #if the units are m/s
    ...
    #NOW WE CALCULATE THE MASS#  
            ...
            HI_mass=(2.35*10**5)*(distance**2)*pixelsize*np.nansum(darray[box[0]:box[1],box[2]:box[3]])/beamarea #compute the HI masss
    else:
...
#############################################################################################
def flux(data,units=None,beamarea=None,pixelres=None,spectralres=None,box=None,verbose=False):
    ...
    #NOW WE CALCULATE THE FLUX#  
            ...
            flux=np.nansum(darray[region[0]:region[1],region[2]:region[3]])*spectralres*pixelsize/beamarea #calculate the flux
    else:
...
def create_config(name='default_parameters'):
    ...
    with open(name+'.par',mode='w') as configfile: #equivalent of saying configfile=open()
        configfile.write(";configuration file for nhicci. Use ';' or '#' to comment the parameters you don't need/know\n"
            ...
            '\n'
            '[CUBEPAR]\n'
            'robust		=	   #robust parameter of the data cube\n'
            'spectralres	=	   #spectral resolution of km/s (default: None)\n'
            'pixelres	=	   #pixel resolution in arcsec (default: None)\n'
            'rms         =	   #root-mean-square value in Jy/beam (default: None)\n'
            '\n'
            '[BEAM]\n'
